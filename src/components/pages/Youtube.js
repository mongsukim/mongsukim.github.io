...
function YouVideo() {
  ...
  const search = (query) => {
    ...
    fetch(`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=25&q=${query}&type=video&key=AIzaSyAXNTB1NWl6ppjiGwp9bg2RnUlbhhIqF0M`, requestOptions)
  ...
  useEffect(() => {
    ...
    fetch(
      "https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=30&q=webstoryboy&type=video&key=AIzaSyAXNTB1NWl6ppjiGwp9bg2RnUlbhhIqF0M",
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => setVideos(result.items))
